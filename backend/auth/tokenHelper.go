...
func GenerateAccessToken(name string) (signedToken string,  err error) {
	claims := &SignedDetails{
		Name:      name,
		StandardClaims: jwt.StandardClaims{
			ExpiresAt: time.Now().Local().Add(time.Minute * time.Duration(1)).Unix(),
...
func UpdateAccessToken(signedToken string,userId uint) {
	_, err := db.DB.Exec("UPDATE users SET token = $1 WHERE uid = $2", signedToken, userId)
	if err != nil {
		panic(err)
	}
